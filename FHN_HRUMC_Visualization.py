...
def update_I(val):
    ...
    # PLOT 2
    # Create mapping (-4.25 to -0.75 in increments of 0.01 -> 0 to 349)
    position = np.interp(I_slider.val, [-4.25, -0.75], [0, 349])
...
# Create animation for the I_slider
def on_key_press(event):
    global anim, animation_running, I_slider
    if event.key == '1' and not animation_running:
        # Signal that the animation is beginning and stop manual slide inputs
        animation_running = True
        I_slider.set_active(False)

        # Start from the beginning
        I_slider.set_val(-4.25)
        ...
        return
def animate(frame):
    global animation_running, I_slider
    # Calculate the new slider value
    new_val = I_slider.val + 0.05

    # Break out of the function when done
    if new_val > I_slider.valmax:
        I_slider.set_active(True)
        animation_running = False
        return anim.event_source.stop()

    # Update the I_value
    I_slider.set_val(new_val)

    # Pass new values to update functions
    update_I(new_val)
    update_t(new_val)
...
# Boolean to prevent interrupting the animation
animation_running = False
...
for I in np.arange(-4.25, -0.75, 0.01):
...
I_slider = Slider(ax_I_slider, "I (External Current)", -4.25, -0.75, valinit=-4.25, valstep=0.01)
